/*
 * sha256.c — SHA-256 (FIPS 180-4), implementación de dominio público.
 * Ver sha256.h para contexto de uso en este proyecto.
 */
#include "sha256.h"
#include <string.h>
...
static void sha256_transform(sha256_ctx_t *ctx, const uint8_t data[SHA256_BLOCK_SIZE])
{
...
void sha256_final(sha256_ctx_t *ctx, uint8_t hash[SHA256_DIGEST_SIZE])
{
    ...
    /* Padding: 0x80, ceros, y la longitud en bits (big-endian, 8 bytes)
     * al final del último bloque de 64 bytes (o de uno extra si no
     * entra el padding completo en el bloque actual). */
    if (ctx->datalen < 56) {
        ctx->data[i++] = 0x80;
